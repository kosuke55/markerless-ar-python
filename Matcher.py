...
class Matcher:
    ...
    def getCorrespondence(self):
        ...
        if self.alg == 'orb':
            orb = cv2.ORB_create()
            # kp2: keypoints from the frame/captured image
        ...
        # [ ..., [<DMatch 0x104a34c30>, <DMatch 0x104a34c50>], ... ]
        ...
        if len(good) > MIN_MATCH_COUNT:
            ...
            # M is the transformation matrix, thresholding at 5
            M, mask = cv2.findHomography(src_pts, dst_pts, cv2.RANSAC, 5.0)
            matchesMask = mask.ravel().tolist()

            h, w = self.roi.image.shape
            # 4 corners of the roi pattern image
    ...
    def computePose(self, src, dst):
        ...
        retval, rvec, tvec
        '''
        retval, rvec, tvec = cv2.solvePnP(src.astype(np.float32),
